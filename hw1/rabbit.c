...
rabbit_t *add_rabbit(rabbit_list_t *wclist, char *word)
{
  ...
  if (rabbit == NULL)
  {
    rabbit = (rabbit_t *)malloc(sizeof(rabbit_t));
    rabbit->breed = malloc(strlen(word) + 1);
    strncpy(rabbit->breed, word, strlen(word) + 1);
    rabbit->count = 1;
...
size_t total_breeds(rabbit_list_t *rlist) { return list_size(rlist); }
...
{
  bool(*aux_ptr)(const rabbit_t *, const rabbit_t *) = aux;
  return (*aux_ptr)(list_entry(ewc1, rabbit_t, elem),list_entry(ewc2, rabbit_t, elem));
}
